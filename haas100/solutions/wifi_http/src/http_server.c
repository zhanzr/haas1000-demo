...
static void api_info(int fd)
{
    ...
    if (n && !ip4_addr_isany(netif_ip4_addr(n))) {
        ip4addr_ntoa_r(netif_ip4_addr(n), lan, sizeof(lan));
    }
...
static void handle_client(int fd)
{
     ...
     * adjacent kernel objects (this caused a memory-corruption crash in
     * eth_http). */
...
static void http_server_task(void *arg)
{
    ...
    if (srv < 0) {
        printf("wifi_http: socket failed\r\n");
        return;
    }
    ...
    if (lwip_bind(srv, (struct sockaddr *)&addr, sizeof(addr)) != 0) {
        printf("wifi_http: bind failed\r\n");
        lwip_close(srv);
        return;
    }
    if (lwip_listen(srv, 8) != 0) {
        printf("wifi_http: listen failed\r\n");
        lwip_close(srv);
        return;
    }

    printf("wifi_http: listening on :%d\r\n", HTTP_PORT);
    ...
    for (;;) {
        ...
        if (c >= 0) {
            /* Disable Nagle so lwIP sends each segment immediately; on the
             * CH395 MAC-raw link delayed ACKs would otherwise stall large
             * response bodies. */
            int one = 1;
